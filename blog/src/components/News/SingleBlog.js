...
function SingleBlog() {
    ...
    fetch(`http://localhost:8081/single-blog/${id}`)
      .then((response) => {
        if (!response.ok) {
    ...
    //     const allDatas = data.find((allDatas) => allDatas.id === id);
    //     console.warn(allDatas);
    //     const {author_Name, author_Email} = allDatas; 

    // useEffect(() => {
    //     axios.get(`http://localhost:8081/single-blog/${id}`)
    //       .then((res) => res.json())
    //       .then((data) => {
    //         setData(data);
    //         console.warn('Fetched data:', data);
    ...
    // if (Object.keys(data).length === 0) {
        // Data is not available yet, show loading or some placeholder
        // return <p>Loading...</p>;
        ...
        <>
            ...
            <div className="single-post-area">
                <p>
                    ...
                    {data.blog_text_First}
                   
                </p>
                <div className="post-thumb">
                    <img src={`http://localhost:8081/images/` + data.blog_image_1} alt="" />
                </div>
                <h4 className="article-title">{data.blog_heading}</h4>
                <p>
